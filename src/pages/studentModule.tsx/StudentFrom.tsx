...
// 👇 NEW — a student keeps the SAME admission number across every academic
// year (your getStudentById response has it both at the top level and on
// each academicInformation entry, e.g. "RGN002"). When adding a brand new
// academic-info block (e.g. promoting to next year) for an EXISTING
// student, we reuse whatever admissionNo is already sitting in the form's
// other academicInformation entries instead of leaving it blank — since
// this field is disabled/system-generated, the user can never type it in
// themselves.
const getExistingAdmissionNo = (form: FormInstance): string | number | undefined => {
  const currentEntries = form.getFieldValue("academicInformation") || [];
  const withAdmissionNo = currentEntries.find(
    (entry: any) => entry?.admissionNo !== undefined && entry?.admissionNo !== null && entry?.admissionNo !== ""
  );
  return withAdmissionNo?.admissionNo;
};
            ...
            {(fields, { add, remove }) => (
              <>
                {fields.map(({ key, name, ...restField }) => (
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4">
                      ...
                      >
                        <InputNumber style={{ width: "100%" }} placeholder="Auto-generated by system" disabled />
                      </Form.Item>
                      <Form.Item
                        {...restField}
                ...
                <Button
                  ...
                  onClick={() =>
                    add({
                      // 👇 CHANGED — was `admissionNo: undefined`. Now reuses
                      // the admission number already present on the
                      // student's other academic-info entries, since this
                      // field is disabled and the user can't type it in.
                      // For a brand-new student with no prior entries yet,
                      // getExistingAdmissionNo(form) returns undefined and
                      // this behaves exactly as before (blank, backend fills
                      // it in on save).
                      admissionNo: getExistingAdmissionNo(form),
                      admissionDate: null,
                      standard: undefined,
                      division: undefined,
                      rollNo: "",
