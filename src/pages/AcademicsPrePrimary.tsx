    ...
    <div className="acad-pr-page" style={{ fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
      ...
      <div style={{ background: "#1569ad", position: "relative" }}>
        ...
        <div className="acad-title-wrap" style={{ maxWidth: "1200px", margin: "0 auto", padding: "0 40px 26px" }}>
          ...
          <h1 style={{ color: "#fff", fontSize: "34px", fontWeight: 500, margin: 0 }}>
            {classRoom?.classRoomName || "Pre-Primary School"}
          </h1>
          <p style={{ color: "rgba(255,255,255,0.85)", fontSize: "14px", marginTop: "6px" }}>
            {classRoom?.description || "Playgroup · Nursery · Junior KG (LKG) · Senior KG (UKG)"}
          </p>
        </div>
      </div>
      ...
      {classRoom?.academicYearDTOS && classRoom.academicYearDTOS.length > 0 && (
        <div className="acad-section" style={{ padding: "50px 80px" }}>
          ...
          >
            {/* LEFT: Financial Year accordion — all years live in one list */}
              ...
              {classRoom.academicYearDTOS.map((year) => {
                ...
                return (
                  <div key={year.academicYearId} style={{ borderTop: "1px solid #e2e8f0" }}>
                    ...
                    {/* subScreenName list nested right under this year, inline in the left column */}
                    {isExpanded && (
            ...
            {/* RIGHT: fixed slot — only its content swaps when a subScreenName is clicked */}
                ...
                if (!activeSubScreen) {
                  return (
                    <p style={{ fontSize: "13px", color: "#777" }}>
                      {/* Select a financial year, then a record, to view details here. */}
                    </p>
                  );
                }
      ...
      {/* Subjects Offered — now fully dynamic from subjectDTOList */}
        ...
        <div className="acad-subjects-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: "20px" }}>
          {classRoom?.subjectDTOList?.map((subj, i) => (
            <div
              ...
              style={{ background: "#fff", borderTop: "4px solid #1569ad", borderRadius: "6px", padding: "14px 18px", boxShadow: "0 4px 14px rgba(0,0,0,0.06)" }}
            >
              <h3 style={{ color: "#1a3a6b", fontSize: "16px", fontWeight: 700, marginBottom: "4px" }}>{subj.subjectName}</h3>
